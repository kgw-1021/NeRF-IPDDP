# IPDDP with Gaussian local costmap + line search + convergence check + smoothness regularization
...
def ipddp_step(x0, u_seq, goal, density_field, grid_x, grid_y, T, dt):
    ...
    for t in range(T):
        ...
        Q, R, q, r, _ = cost_quadratic_approx(x_seq[t], u_seq[t], goal, cost_fn)
        fx_seq.append(fx)
        fu_seq.append(fu)
        Q_seq.append(Q)
        R_seq.append(R)
...
# ------------------- 실행 및 시각화 -------------------
def run_ipddp_full(x0, goal, density_field, grid_x, grid_y, T=200, dt=0.2, max_iters=20,
                   alpha_list=[1.0, 0.8, 0.5, 0.3, 0.1], convergence_tol=1e-9, lambda_smooth=10.0):
    output_dir = "ipddp_iterations_full"
    ...
    for i in range(max_iters):
        x_seq, u_seq_candidate, costmap = ipddp_step(x0, u_seq, goal, density_field, grid_x, grid_y, T, dt)
        cost_fn = build_cost_fn_from_costmap(costmap, grid_x, grid_y)
        ...
        for alpha in alpha_list:
            ...
            for t in range(T):
                x = x_test[-1]
                u = u_test[t]
                total_cost += 0.1 * np.dot(u, u) + 10.0 * cost_fn(x)
                if t > 0:
                    du = u_test[t] - u_test[t - 1]
                    total_cost += lambda_smooth * np.dot(du, du)
                x_test.append(dynamics(x, u, dt))

            x_test = np.array(x_test)
            total_cost += 10.0 * np.sum((x_test[-1][:2] - goal[:2])**2)
        ...
        if cost_reduction < convergence_tol:
            converged_iter = i + 1
            print(f"✅ Converged at iteration {converged_iter} (ΔJ = {cost_reduction:.6f})")
            break
...
x_seq_final, u_seq_final, gif_path_final = run_ipddp_full(start, goal, density_field, grid_x, grid_y)
